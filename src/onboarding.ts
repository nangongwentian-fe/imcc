...
async function onboardLark(profileName: string): Promise<ProfileConfig['channel']> {
  p.note(
    [
      ...
      `4. 「权限管理」→ 搜索并开通：`,
      `   ${pc.green('卡片相关')}`,
      `   ${pc.green('✓')} ${pc.bold('cardkit:card:read')}（获取卡片信息）`,
      `   ${pc.green('✓')} ${pc.bold('cardkit:card:write')}（创建与更新卡片）`,
      `   ${pc.green('✓')} ${pc.bold('cardkit:template:read')}（获取卡片模板信息）`,
      `   ${pc.green('消息相关')}`,
      `   ${pc.green('✓')} ${pc.bold('im:message')}（获取与发送单聊、群组消息）`,
      `   ${pc.green('✓')} ${pc.bold('im:message:group_at_msg:readonly')}（接收群聊中 @ 机器人消息事件）`,
      `   ${pc.green('✓')} ${pc.bold('im:message:group_msg')}（获取群组中所有消息，敏感权限）`,
      `   ${pc.green('✓')} ${pc.bold('im:message:p2p_msg:readonly')}（读取用户发给机器人的单聊消息）`,
    ].join('\n'),
    '飞书应用配置',
  );
